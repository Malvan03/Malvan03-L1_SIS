...
public class GestionConductoresServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            // Cálculo de antigüedad para cada conductor
            for (Conductor c : lista) {
                if (c.getFechaIngreso() != null) {
                    Date fecha = c.getFechaIngreso();
                    LocalDate ingreso;
                    if (fecha instanceof java.sql.Date) {
                        ingreso = ((java.sql.Date) fecha).toLocalDate();
                    } else {
                        ingreso = fecha.toInstant().atZone(java.time.ZoneId.systemDefault()).toLocalDate();
                    }
                    Period period = Period.between(ingreso, LocalDate.now());
                    String antiguedad = period.getYears() + " años, " +
