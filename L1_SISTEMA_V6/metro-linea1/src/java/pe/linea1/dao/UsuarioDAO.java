...
public class UsuarioDAO {
    ...
    // Validar usuario por dni y password encriptado, bloqueo temporal e inactivos
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (bloqueadoHasta != null && bloqueadoHasta.after(new java.util.Date())) {
                        return null; // Usuario bloqueado temporalmente
                    }
                    // ¿Está activo?
                    ...
                    if (BCrypt.checkpw(password, hashAlmacenado)) {
                        // Éxito: resetear intentos y bloqueo, actualizar fecha login
    ...
    // Verifica si un usuario está bloqueado temporalmente por intentos fallidos
    ...
    // Actualiza los datos del usuario (excepto password y rol)
    ...
    // Activa o desactiva (inactiva) un usuario
    ...
    // Busca usuario por DNI (devuelve objeto Usuario)
    ...
    // Utilidad interna: convierte un ResultSet en objeto Usuario
